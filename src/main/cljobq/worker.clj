...
(defn ^:no-doc work-loop
  ...
  [db queue-name qdef stop-ch app-context]
            ...
            (if (> attempt max-attempts)
              ...
              (try
                (let [f (util/str->fnvar actor)
                  ...
                  (if (= app-context ::no-context)
                    ;; Legacy behaviour - no :app/context provided in context.
                    (apply f args)
                    ;; :app/context provided - it's passed as first argument
                    ;; to the job function.
                    (apply f app-context args))
                  (if interval
                    (job/reenqueue-job!
                      db {:id id
                          :run-at (util/->offset-time-zulu
...
(defn ^:no-doc start-queue-threads [context queue-name qdef stop-ch]
  (let [{:keys [num-threads]} qdef
        {:keys [db] app-context :app/context, :or {app-context ::no-context}} context]
    (repeatedly
      num-threads
      (fn []
        (async/thread
          (loop [exited? false]
            (when-not exited?
              (recur
                (try
                  (work-loop db queue-name qdef stop-ch app-context)
                  true
                  (catch Exception e
